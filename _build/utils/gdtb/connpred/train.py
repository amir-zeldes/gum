...
class DataTrainingArguments:
    ...
    text_column: Optional[str] = field(
        default=None,
        metadata={"help": "The name of the column in the datasets containing the full texts (input)."},
    )
...
def main():
    ...
    send_example_telemetry("run_trainer", model_args, data_args)
    ...
    # summaries (unless you specify column names for this with the `text_column` and `output_column` arguments).
    #
    ...
    if data_args.output_column is None:
        output_column = dataset_columns[1] if dataset_columns is not None else column_names[1]
    else:
    ...
    def preprocess_function(examples):
        ...
        for i in range(len(examples[text_column])):
            if examples[text_column][i] and examples[output_column][i]:
    ...
    kwargs = {"finetuned_from": model_args.model_name_or_path, "tasks": "pdtb"}
    if data_args.dataset_name is not None:
